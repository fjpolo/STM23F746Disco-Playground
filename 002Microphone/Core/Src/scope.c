...
void ScopeLCD_Show(short * pData1, short * pData2)
{
  ...
  //if((oscConfig.numberOfSamples*2) - triggerSample > LCD_GetXSize()-GRAPH_BORDER_L)
    numToShow = MIN(BSP_LCD_GetXSize()-GRAPH_BORDER_L, NUM_SAMPLES);
  //else
  /*calculate how many samples application can show*/	
    //numToShow = (oscConfig.numberOfSamples*2) - triggerSample;
  /*scale pData buffer*/
  ScopeScale(pData1,pData1, 4096,  MIN(BSP_LCD_GetYSize(), NUM_SAMPLES), numToShow);		// scale the data amplitude for show
  ScopeScale(pData2,pData2, 4096,  MIN(BSP_LCD_GetYSize(), NUM_SAMPLES), numToShow);		// scale the data amplitude for show
  /*Plot Y axis*/
